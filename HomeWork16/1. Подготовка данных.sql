...
(
    ...
    EmployeeId INT NOT NULL, --Сотрудник, обработавший данную позицию в заказе.
...
INSERT INTO LineOrder (OrderId, PriceProduct, PriceClient, EmployeeId, ProductId, Quantity)
SELECT 1 AS OrderId, NVL(p.price, tp.price) AS PriceProduct,  NVL(p.price, tp.price) AS PriceClient, 1 AS EmployeeId, p.productid, 4 AS Quantity
...
INSERT INTO LineOrder (OrderId, PriceProduct, PriceClient, EmployeeId, ProductId, Quantity)
SELECT 1 AS OrderId, NVL(p.price, tp.price) AS PriceProduct, NVL(p.price, tp.price) AS PriceClient, 3 AS EmployeeId, p.productid, 4 AS Quantity
